...
public abstract class Customer {
	...
	public Customer(String name, String mobileNumber, int locationX, int locationY, String type) throws CustomerException{
		// TO DO
		if (name.trim().length() <= 1) { //name exception
			throw new CustomerException("Error: Invalid customer's name");
		}
		if (mobileNumber.trim().length() < 10 || mobileNumber.trim().length() > 10 ||
				mobileNumber.trim().startsWith("0") == false) { //mobile number exception
			throw new CustomerException("Error: Invalid mobile number");
		}
		if (Integer.toString(locationX).trim().length() == 0 || Math.abs(locationX) > maxDistance) { //coordinates expcetion
			throw new CustomerException("Error: Invalid location coordinate(s)");
		}
		if (Integer.toString(locationY).trim().length() == 0 || Math.abs(locationY) > maxDistance) { //coordinates expcetion
			throw new CustomerException("Error: Invalid location coordinate(s)");
		}
		if (type.trim().length() == 0) { //customer type exception
			throw new CustomerException("Error: Invalid customer type");
		}
		this.name = name.trim(); this.mobileNumber = mobileNumber.trim();
		this.locationX = locationX; this.locationY = locationY; this.type = type.trim();
	}
